...
def count_ops(op, sparsity, param_bits):
  ...
  if isinstance(op, Conv2D):
    ...
    assert op.kernel_shape[2] == op.kernel_shape[3]
    c_out, c_in, k_size, _ = op.kernel_shape
...
def get_info(op):
  ...
  if isinstance(op, (DepthWiseConv2D, Conv2D)):
    ...
    out_channels, in_channels, kernel_size, kernel_size  = op.kernel_shape
...
class MicroNetCounter(object):
  ...
  def process_counts(self, total_params, total_mults, total_adds,
                     mul_bits, add_bits):
    # counting the number of parameters with a base of 32
